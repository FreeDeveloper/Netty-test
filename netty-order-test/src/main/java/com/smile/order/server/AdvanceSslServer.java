...
import com.smile.order.server.handler.AuthHandler;
...
//        schedule.scheduleAtFixedRate(new MyEventLoopMetricsThread(worker), 0, 5, TimeUnit.SECONDS);
        ...
        System.out.println(signedCertificate.certificate());
        SslContext sslContext = SslContextBuilder.forServer(signedCertificate.certificate(),signedCertificate.privateKey()).build();
